...
segformer_transforms = lazy.load(
    "dsa_helpers.ml.transforms.segformer_transforms", suppress_warning=True
)
tile_utils = lazy.load("dsa_helpers.tile_utils", suppress_warning=True)
...
def evaluate_semantic_segmentation_segformer_group(
    ...
    for group in tqdm.tqdm(groups, desc="Evaluating groups"):
        ...
        for i in range(0, len(group_df), batch_size):
            ...
            # Create dataset for this batch.
            group_dataset = datasets_utils.create_segformer_segmentation_dataset(
                batch_df, transforms=segformer_transforms.val_transforms
            )
        ...
        pred_gdf = tile_utils.merge_tile_masks(
            pred_tile_list, background_label=background_label
        )
        true_gdf = tile_utils.merge_tile_masks(
